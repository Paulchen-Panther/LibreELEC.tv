...
PKG_CMAKE_OPTS_TARGET="-DFLATBUFFERS_BUILD_FLATC=ON \
                       -DFLATBUFFERS_BUILD_FLATHASH=OFF \
...
post_makeinstall_target() {
  ...
  # HACK: we have flatc in $TOOLCHAIN/bin but it seems
  # the one from sysroot prefix is picked when building hyperion. replace it!
  rm -f ${SYSROOT_PREFIX}/usr/bin/flatc
  cp -PR ${TOOLCHAIN}/bin/flatc ${SYSROOT_PREFIX}/usr/bin
}
